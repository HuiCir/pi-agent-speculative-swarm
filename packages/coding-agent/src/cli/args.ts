...
export function parseArgs(args: string[]): Args {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--help" || arg === "-h") {
			...
			result.swarmModel = args[++i];
			result.swarm = true;
		} else if (arg === "--swarm-agents" && i + 1 < args.length) {
			result.swarmAgents = args[++i]
				.split(",")
				.map((s) => s.trim())
				.filter((name) => name.length > 0);
			result.swarm = true;
		} else if (arg === "--swarm-rounds" && i + 1 < args.length) {
			result.swarmRounds = parsePositiveInteger(args[++i], "--swarm-rounds", result);
			result.swarm = true;
		} else if (arg === "--swarm-max-turns" && i + 1 < args.length) {
			result.swarmMaxTurns = parsePositiveInteger(args[++i], "--swarm-max-turns", result);
			result.swarm = true;
		} else if (arg === "--swarm-timeout-ms" && i + 1 < args.length) {
			result.swarmTimeoutMs = parsePositiveInteger(args[++i], "--swarm-timeout-ms", result);
			result.swarm = true;
		} else if (arg === "--swarm-tool-policy" && i + 1 < args.length) {
			const value = args[++i];
			if (value === "none" || value === "readonly" || value === "all") {
				result.swarmToolPolicy = value;
				result.swarm = true;
			} else {
				result.diagnostics.push({
					type: "error",
...
function parsePositiveInteger(value: string, flag: string, result: Args): number | undefined {
